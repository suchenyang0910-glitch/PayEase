...
| 2.2  | □ 会话绑定：session cookie `Path=/; HttpOnly; Secure; SameSite=None`（因为在 iframe 里必须 SameSite=None；但对应 §4 CSP 只允许 tg 域，降低 CSRF 面）；**绑定 user.id + user.hash_fingerprint + client IP + UA**；任一变更 → 立即 401 强制重登 | 剧本 4：证件越权访问，若会话 token 被盗用跨 UA/跨 IP → 失败       |
