...
describe("HR DemoApp trilingual copy uses exported HR_DEMO_COPY from DemoApp directly", () => {
  afterEach(() => cleanup());

  it("every leaf in HR_DEMO_COPY[zh-CN/en/km] is non-empty, non-placeholder; zh-CN and km differ from en in >=75% by-key leaf pairs", () => {
  ...
  it("language switcher at runtime renders HR_DEMO_COPY title + subtitle + list + detail strings", async () => {
    for (const lang of DEMO_LANGUAGES) {
      cleanup();
      render(
      ...
      await waitFor(() => {
        expect(
          screen.getByRole("heading", { name: copy.list }),
        ).toBeInTheDocument();
        expect(screen.getByText(copy.employerOnly)).toBeInTheDocument();
      });

      fireEvent.click(screen.getByRole("button", { name: copy.reviewDemo }));
      await waitFor(() => {
        expect(
          screen.getByRole("heading", { name: copy.detail }),
        ).toBeInTheDocument();
        expect(screen.getByText(copy.employmentOutcome)).toBeInTheDocument();
